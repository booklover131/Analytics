...
FROM
 ...
 SELECT DISTINCT distinct_id AS shimo_user_id,
   ...
   WHERE ldate>'2018-11-11'
     ...
     AND s_url_path REGEXP '/slide'
     AND s_url NOT regexp 'preload'--访问页面url包含幻灯片url ,不包含预加载

   UNION ALL --11.11前神策历史数据web侧日活跃，访问了幻灯片的
 SELECT DISTINCT distinct_id AS shimo_user_id,
 ...
 SELECT DISTINCT shimo_user_id,
   ...
   WHERE ldate >='2018-12-07'
     ...
     AND cast(fileid AS int) IN --文件表，-5 幻灯片

       (SELECT DISTINCT id
        FROM default.shimo_files
        WHERE ldate=date_add(to_date(now()),-1)
          AND TYPE IN (-5) )
   UNION ALL --小程序sdk打通后打开了幻灯片的日活跃数据,19号上线打开文件的事件
 SELECT DISTINCT shimo_user_id,
                 shimo_device_id,
                 ldate,
                 CASE
