...
FROM
 ...
 SELECT DISTINCT distinct_id AS shimo_user_id,
   ...
   WHERE ldate>'2018-11-11'
     ...
     AND s_url_path REGEXP '/sheet'
     AND s_url NOT regexp 'preload'--访问页面url包含表格url ,不包含预加载

   UNION ALL --11.11前神策历史数据web侧日活跃，访问了表格的
 SELECT DISTINCT distinct_id AS shimo_user_id,
 ...
 SELECT DISTINCT shimo_user_id,
   ...
   WHERE ldate >='2018-12-07'
     ...
     AND cast(fileid AS int) IN --文件表，-1/-3/-4 旧表格/新表格/新表格（modoc）
       ...
       (SELECT DISTINCT id
        ...
        WHERE ldate=date_add(to_date(now()),-1)
          AND TYPE IN (-1,
                       -3,
                       -4) )
   UNION ALL --小程序sdk打通后打开了表格的日活跃数据,19号上线打开文件的事件
 SELECT DISTINCT shimo_user_id,
                 shimo_device_id,
                 ldate,
                 CASE
